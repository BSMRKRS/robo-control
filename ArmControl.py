...
# 	!!!Comment (1.) out if using ArmControl.py for iterating through (x,y)
#
# (2.) Translates (x,y) into the necessary angles of the arm beams for the endp
# be at (x,y)
#
# (3.) From there the angles are translated into the countable events from the
#
...
# the external gear ratio !!! Currently, the motor counts 5462.22 events per
...
plane/automatically comment this section out, and increment through
armKinimatics(x,y) using an outside program to increment through values.
"""
x = float(raw_input("x>"))
y = float(raw_input("y>"))
...
def runMotors(newCount1, newCount2, encoder1, encoder2):
    ...
    count2 = encoder2.Rotary_counter		# Starts Motor1 and Motor2
    if newCount1 > count1:
...
class Encoder(object, Enc_A, Enc_B):
    ...
    def __init__(self):
        ...
        self.Current_A = 1  # This assumes that Encoder initiates while mtr sto
        self.Current_B = 1
    ...
    def rotary_interrupt(self, A_or_B):
        ...
        # if not that means that bouncing caused it Same interrupt as before
