...
motorL = 0
motorR = 1
cameraF = 15 #forward facing camera
cameraB = 14 #backwards facing camera

motorR_forward = 1750
motorR_backward = 1250
motorL_forward = 1750
motorL_backward = 1250
...
def stopAll():
  try:
    RPL.servoWrite(motorL,1490)
    RPL.servoWrite(motorR,1490)
    RPL.servoWrite(cameraF,1480)
    RPL.servoWrite(cameraB,1480)
  except:
    print "error except"
    pass
...
def forwardLeftSpeedChange(change, mn = 1600, mx = 2900):
 ...
 #CAMERA settings
def cameraFclockwise():
    RPL.servoWrite(cameraF, 1510)
def cameraFcounterclockwise():
    RPL.servoWrite(cameraF, 1450)
def cameraBclockwise():
    RPL.servoWrite(cameraB, 1510)
def cameraBcounterclockwise():
    RPL.servoWrite(cameraB, 1450)
...
while True:
  ...
  if ch == '*': # pressing the asterisk key kills the process
    ...
    if ch == 'w':
      ...
      cameraFcounterclockwise()
    elif ch == "u":
      cameraBclockwise()
    elif ch == "j":
      cameraBcounterclockwise()
    else:
      stopAll()
